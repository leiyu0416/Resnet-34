...
class ResNet32(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)  # [8, 64, 56, 56] 是一个由3个Block组成的序列，对输入数据进行特征提取。
        x = self.layer2(x)  # [8, 128, 28, 28] 是一个由4个Block组成的序列，对输入数据进行特征提取。
        x = self.layer3(x)  # [8, 256, 14, 14] 是一个由6个Block组成的序列，对输入数据进行特征提取。
        x = self.layer4(x)  # [8, 512, 7, 7] 是一个由3个Block组成的序列，对输入数据进行特征提取。
        x = self.avgpooling(x)  # [8, 512, 3, 3]对输入数据x进行平均池化，降低数据维度，去除冗余信息。
        x = x.view(x.shape[0], -1)  # [8, 4608]将张量 x 重新调整为一个二维张量，
